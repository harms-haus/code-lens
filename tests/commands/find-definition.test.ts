...
await import("../../src/commands/find-definition.js");

// ── Helpers ────────────────────────────────────────────────────────────────

function getHandler(): Function {
  const handler = registeredHandlers.get("find-definition");
...
describe("find-definition command", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("returns locations with formatted output and count", async () => {
    ...
    expect(result.content[0].text).toContain("Definition found: 2 locations");
  ...
  it("returns ok with 0 locations when LSP returns null", async () => {
    setupPreambleSuccess();
    mockClient.gotoDefinition.mockResolvedValue(null);

    const handler = getHandler();
    const result = (await handler(validParams, mockManager, defaultCwd)) as CommandResult;

    expect(result.isError).toBe(false);
    expect(result.content[0].text).toContain("Definition found: 0 locations");
    expect(result.details).toMatchObject({
      file: "src/index.ts",
      line: 10,
      col: 5,
  ...
  it("returns sanitized error when LSP client throws", async () => {
    setupPreambleSuccess();
    mockClient.gotoDefinition.mockRejectedValue(new Error("Connection lost"));

    const handler = getHandler();
    const result = (await handler(validParams, mockManager, defaultCwd)) as CommandResult;

    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain("Failed to find definition");
